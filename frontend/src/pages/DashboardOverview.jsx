...
export default function DashboardOverview({ setActiveTab }) {
  ...
  const inReviewNotesCount = (notebookEntries || []).filter(n => n.status === 'In Review').length;
  ...
  const calendarItems = [
    { date: 'Today', event: 'CRISPR Transfection Plate Review', time: '10:00 AM', lab: 'Genomics Wing', tag: 'High Priority' },
    { date: 'Tomorrow', event: 'Centrifuge Yield Spectrometry', time: '02:30 PM', lab: 'Biochem Core', tag: 'Scheduled' },
    { date: 'Jun 18', event: 'Project Artemis Peer Audit Check', time: '11:00 AM', lab: 'Conference Room B', tag: 'Regulatory' },
    { date: 'Jun 20', event: 'Annual FDA 21 CFR Part 11 Inspection', time: '09:00 AM', lab: 'Central R&D HQ', tag: 'Inspection' }
  ];
      ...
      return (
        <div className="bg-slate-900/95 border border-slate-800 text-white rounded-xl p-3 shadow-xl text-xs space-y-1 backdrop-blur-xl">
          ...
          {payload.map((pld, index) => (
            <div key={index} className="flex justify-between items-center gap-6 font-semibold" style={{ color: pld.color }}>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white border border-slate-200 p-5 rounded-2xl h-28 flex justify-between">
    ...
    <div className="space-y-6 animate-fade-in-up font-sans">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-850 to-teal-950 p-6 sm:p-8 text-white shadow-xl border border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        ...
        <div className="relative z-10 space-y-2 max-w-xl">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-teal-500/20 text-teal-300 border border-teal-500/30">
              <ShieldCheck className="w-3 h-3" />
              21 CFR PART 11 LEDGER ACTIVE
            </span>
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            Your laboratory cluster is operating normally. All cryptographic signature verification checks passed without exceptions.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl shadow-xs flex justify-between items-start hover:border-teal-500/40 transition-colors group">
          <div>
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">Active Projects</span>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 mt-1 font-mono">{activeProjectsList.length}</div>
            <div className="text-[11px] text-slate-500 mt-1.5 flex items-center gap-1 font-medium">
              <span className="text-teal-700 font-bold">✓ {completedProjectsCount} completed</span>
              this semester
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl shadow-xs flex justify-between items-start hover:border-teal-500/40 transition-colors group">
          <div>
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">Notebook Entries</span>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 mt-1 font-mono">{(notebookEntries || []).length}</div>
            <div className="text-[11px] text-slate-500 mt-1.5 flex items-center gap-1.5">
              ...
              <span className="text-slate-400 text-[10px]">Signed: {(notebookEntries || []).filter(n => n.status === 'Signed').length}</span>
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl shadow-xs flex justify-between items-start hover:border-teal-500/40 transition-colors group">
          <div>
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">Shared Assets</span>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 mt-1 font-mono">{(sharedResources || []).length}</div>
            <div className="text-[11px] text-slate-500 mt-1.5 flex items-center gap-1 font-medium">
              <span className="text-teal-700 font-bold">✓ Multi-node access</span>
              calibrated
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl shadow-xs flex justify-between items-start hover:border-teal-500/40 transition-colors group">
          <div>
            <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">Audit Security</span>
            <div className="mt-2.5">
              <span className="text-[10px] font-mono font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full inline-flex items-center">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5 animate-pulse" />
                SOC 2 + Part 11 Active
              </span>
            </div>
            <div className="text-[10px] text-slate-400 mt-2">Zero audit anomalies reported</div>
          </div>
          <div className="bg-purple-50 text-purple-600 p-2.5 rounded-xl border border-purple-150 group-hover:scale-105 transition-transform">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200/80 rounded-2xl p-5 sm:p-6 shadow-xs">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-6">
            <div>
              <h3 className="font-extrabold text-slate-900 text-sm">Weekly Bench Output & Digital Logging</h3>
              <p className="text-[11px] text-slate-400 mt-0.5">Tracking investigator bench hours vs digital notebook submissions</p>
            </div>
          ...
          <div className="h-68">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={productivityData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" name="Lab Hours" dataKey="Lab Hours" stroke="#0D9488" strokeWidth={2.5} fillOpacity={1} fill="url(#tealGrad)" />
                <Area type="monotone" name="Data Entries" dataKey="Data Entries" stroke="#6366F1" strokeWidth={2.5} fillOpacity={1} fill="url(#blueGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white border border-slate-200/80 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col justify-between">
          <div>
            <h3 className="font-extrabold text-slate-900 text-sm">Project Completion Indexes</h3>
            <p className="text-[11px] text-slate-400 mt-0.5">Progress percentages against assigned milestones</p>
          </div>
          ...
          <div className="h-48 my-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={allocationData} margin={{ left: -25, right: 10 }}>
                ...
                <Tooltip content={<CustomChartTooltip />} />
                <Bar name="Progress %" dataKey="Progress" fill="#0D9488" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs space-y-4">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <h3 className="font-bold text-slate-900 text-xs flex items-center gap-2">
              <CalendarIcon className="w-4 h-4 text-teal-600" /> Research Calendar
            </h3>
          ...
          <div className="space-y-3">
            {calendarItems.map((item, idx) => (
              <div key={idx} className="flex justify-between items-start border-b border-slate-100 last:border-0 pb-3 last:pb-0 text-xs">
                <div>
                  <h4 className="font-bold text-slate-800 text-xs">{item.event}</h4>
                  <p className="text-[10px] text-slate-400 mt-0.5">{item.lab}</p>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-[9px] font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded block">
                    {item.date}
                  </span>
                  <span className="text-[8px] font-mono text-slate-400 mt-1 block">{item.time}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Card 2: Compliance Audit Trail */}
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs space-y-4">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <h3 className="font-bold text-slate-900 text-xs flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Compliance Audit Trail
            </h3>
            <button 
              type="button"
            ...
            >
              All Logs <ArrowRight className="w-3 h-3 ml-0.5" />
            </button>
          </div>
          ...
          <div className="space-y-3">
            {(auditLogs || []).slice(0, 4).map((log) => (
              <div key={log.id} className="flex items-start gap-2.5 text-xs">
                <div className="h-6 w-6 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center text-[10px] font-mono shrink-0 mt-0.5">
                  ⚡
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-slate-800 truncate">{log.action}</div>
                  <div className="text-[10px] text-slate-400 truncate mt-0.5">{log.target}</div>
                  <div className="flex justify-between items-center text-[9px] font-mono text-slate-400 mt-1">
                    <span>{log.user ? (log.user.split(' ')[1] || log.user) : 'User'}</span>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs space-y-4">
          ...
          <div className="space-y-2.5">
            {(sharedResources || []).slice(0, 4).map((res) => (
              <div key={res.id} className="p-3 bg-slate-50 border border-slate-200/70 rounded-xl flex items-center justify-between gap-3 text-xs">
                <div className="min-w-0">
                  <div className="font-bold text-slate-800 truncate">{res.name}</div>
                  <div className="text-[10px] text-slate-400 mt-0.5">Owner: {res.owner ? (res.owner.split(' ')[1] || res.owner) : 'PI'}</div>
                </div>
                <span className={`px-2 py-0.5 rounded text-[8px] font-mono font-bold border ${
                  res.permission === 'Owner' ? 'bg-teal-50 text-teal-700 border-teal-200' : 
                  res.permission === 'Editor' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                }`}>
                  {res.permission}
                </span>
              </div>
